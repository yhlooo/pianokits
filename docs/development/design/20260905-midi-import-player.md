...
3. **持久化文件列表**：文件名（列表中去掉 `.mid`/`.midi` 后缀）、大小、导入时间；刷新后列表仍在；支持删除（列表内容是导入时的快照）。
...
**读取**：点击条目直接读 `bytes` 快照，无任何权限流程。快照语义是方案 B 的已知取舍：原文件在磁盘上的后续修改不影响列表内容。导入后顺带调 `navigator.storage.persist()` 降低驱逐风险（Safari 有 7 天无交互主动驱逐）。
...
- 视图由**瀑布 / 乐谱两个图标开关**决定：都开 = 分屏（宽屏左右排，窄屏自动变上下排）、
  只开瀑布 = 瀑布流、只开乐谱 = 五线谱，两个开关不能同时关闭（关闭唯一开启的会自动开启另一个）；
- 文件列表项 hover 出删除（触屏无 hover，删除常显）；
...
| 内容副本是快照，原文件修改不生效                      | 方案 B 的已知取舍（已决策）；未来可升级方案 C                               |
...
- 2026-09-12 ⑧顶栏名称：「MIDI 播放器」→「**播放 / 练习**」（工具 id 与 URI 不变，仍为 `midi-player` / `/midi-player`）。
- 2026-09-06 ⑦当前文件经 URL query 持久化：播放器把当前打开的**文件名**写入 query
  （`?file=<name>`，`replaceState` 不新增历史记录），刷新或切换工具后再进入时据此恢复选中文件；
  删除当前文件时清除该参数。
