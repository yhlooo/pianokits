...
- 不改动调试开关的既有语义：`?debug=1` 仍是调试工具**是否加载/可见**的开关，与路径正交。

### 1.3 验收标准（可测试）

- 访问 `/` 会校正 URL 为 `/midi-player` 并挂载 MIDI 播放器。
- 访问 `/midi-player` 挂载 MIDI 播放器；刷新后仍在同一页面。
- `?debug=1` 时访问 `/midi-keyboard` 挂载 MIDI 键盘；刷新后仍在 MIDI 键盘页。
- 点击页签/调试下拉项会更新地址栏路径，同时**保留** `?debug=1` 等 query（调试模式跨工具切换保持）。
- 浏览器后退/前进按历史路径切换工具。
- 未知路径或非调试模式访问调试工具路径时，回退到首个常规工具并校正 URL。
...
| 3   | 调试/常规区分 | URI 不区分；`?debug=1` 仍控制调试工具是否加载                                             | 满足「URI 上不区分调试与普通工具」；同时保留调试代码不进入生产路径的既有决策                 |
| 4   | query 处理   | 切换工具只改 pathname，**保留** `search`（query）与 `hash`                              | `?debug=1` 需要跨导航保留，否则调试工具刷新即回退                                          |
...
main.ts ── createShell(root)
  └─ shell.ts（路由器 + 外壳）
       ├─ 启动：consumeRedirectPath() → 依 debug 开关懒加载调试工具 → routeFromLocation(true)
...
- 常规工具（`src/tools.ts`）与调试工具（`src/debug/tools.ts`，debug 开启后才加载）合并解析：
...
- 调试工具代码仍在 `isDebugEnabled()` 为真时才被 import，非调试路径访问 `/midi-keyboard` 视为
  未知路由而回退到首个常规工具。
