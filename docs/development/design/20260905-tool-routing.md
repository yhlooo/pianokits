...
- 每个工具对应一个稳定的路径（如「播放 / 练习」`/midi-player`、「录音」`/midi-recorder`；
  工具**显示名可改**而 id/路径不变——如 2026-09-12 把 `midi-player` 的名称从「MIDI 播放器」改为「播放 / 练习」）；
...
- 访问 `/` 会校正 URL 为 `/midi-player` 并挂载「播放 / 练习」。
- 访问 `/midi-player` 挂载「播放 / 练习」；刷新后仍在同一页面。
- 访问 `/midi-keyboard` 挂载 MIDI 键盘调试页；刷新后仍在 MIDI 键盘页。
