...
1. 「播放 / 练习」工具（播放坞）右下角添加**钢琴状态图标**，进入页面自动连接 MIDI 钢琴键盘，
...
- ~~**不镜像延音踏板**（CC64）：走带仍不向输出端口调度 CC，键盘音源听感无踏板；后续
  ...
  —— **2026-09-13 已实现**（设计文档 `20260913-pedal-sound-path.md`）：走带把 CC64/66/67 镜像到
  输出端口（`MidiOutputSink.scheduleControlChange`），机内引擎按踏板烘焙发声时值，练习中把实时
  CC 原样回送；练习门控期间仍**不**镜像文件踏板（踏板由用户负责）；
