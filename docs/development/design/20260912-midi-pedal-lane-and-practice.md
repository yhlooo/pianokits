...
- ~~不把踏板数据镜像到 MIDI 输出端口（键盘音源的 CC64 调度仍是非目标，见 `20260906`§6）；~~
  —— **2026-09-13 已实现**（设计文档 `20260913-pedal-sound-path.md`）：走带镜像曲目 CC64/66/67、
  机内引擎按踏板烘焙发声时值、练习中回送实时 CC；**练习门控期间仍不镜像文件踏板**（踏板由用户负责），
  并新增"延长音符发声时值"的延音语义（`core/midi/pedals.ts` 的 `soundingEndsUnderSustain`）；
- 五线谱不画踏板记号（记谱只沿用 CC64 延长长音的既有语义；该语义的判定条件已由
  `20260913-pedal-sound-path.md` §3.1 修正为 Magenta.js 口径——键抬起时踩着即延长）；
