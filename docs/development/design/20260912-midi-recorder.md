...
- 状态：**正式生效（2026-09-12 实现；2026-09-13 由
  ...
  2026-09-13 再由 `20260913-recorder-auto-record.md` 扩展：演奏自动开始录制、静默 3 秒自动暂停
  与音轨中央状态提示）**
...
| R18 | （2026-09-13 追加）未播放也未录制时按任意键自动开始录制、其中静默 3 秒自动暂停、音轨正中央的状态提示（未连接键盘 / 全空音轨可自动录制） | **`20260913-recorder-auto-record.md`**：空闲时 Note On 即开始录制（手动录制不受自动暂停影响）；中央提示 = 未连接 →「未连接 MIDI 键盘」、`mode === 'idle' && !hasContent` →「在 MIDI 键盘上演奏可自动开始录制」，其余不显示（§3.3、§3.4） |
...
自动录制（2026-09-13 追加，见 `20260913-recorder-auto-record.md` §3.1）：`autoRecord`（本次录制是否
由演奏自动开启）、`lastInputAt`（静默计时基准）、`scrubbing`（音轨拖动中，不自动开录）。
...
| `toggleRecord()`                      | 未连接 → 空操作；从 `position` 起一次 pass（覆盖录制，**手动录制**：不会自动暂停）；再点 → 收尾按住的键 + `commitPass`、停表                        |
| `clear()`                             | 结束：停表、止音、丢弃音符与 pass 状态、位置归零（二次确认在装配层）                                                                                |
| `beginScrub()`/`scrub()`/`endScrub()` | 挂起 → 只改位置 → 从新位置继续；录制中在拖动起点 `commitPass`、松手后从新位置 `beginPass`（D4）；拖动期间不自动开录（R18）                          |
...
| `onNote(ev)`                          | 回送到键盘音源（D6）；空闲（未播放也未录制）时 Note On 先自动开始录制（R18）；录制中且未挂起时按 Note On/Off 建/收音符（收尾的音符进 `_passNotes`） |
...
- **绘制顺序**：背景渐变 → 黑键行底色 → 每行浅横线（C 行更亮）→ 每秒淡竖线（5 秒略亮）→
  ...
  **画布内不画任何说明文字**（2026-09-12 用户口径，2026-09-13 由 R18 修订）：提示信息只出现在
  控件层——按钮 Tips，以及音轨正中央那一条状态提示（DOM 浮层 `.recorder__hint`：未连接键盘时说
  「未连接 MIDI 键盘」；已连接 + 空闲 + 全空音轨时说「在 MIDI 键盘上演奏可自动开始录制」；
  其余情况不显示，见 `20260913-recorder-auto-record.md` §3.2），画布留给音符
  （此前的"请先连接 MIDI 键盘"、"按 ● 开始录制…"两处画在画布里的居中提示均已移除）。
...
| 录制/暂停    | ● / ⏸ | 已连接键盘                   | 从线位置录制/暂停（手动录制：不会自动暂停）；空闲时直接演奏也会开始录制（自动录制：静默 3 秒自动暂停，R18）。禁用时 Tips：`请先连接 MIDI 键盘` 等 |
...
- 自动录制与中央提示（2026-09-13 追加，R18）：`recorder.test.ts` 的「演奏自动录制」一组——空闲按键
  即开录、单独踩踏板不触发、静默 3 秒自动暂停（2.9s / 3.1s 边界、位置停在超时处、定时器停掉）、
  自动暂停后再次演奏继续、手动录制不受影响、自动录制可手动暂停、按住的键与踩住的踏板不算静默、
  回放中与拖动中按键不录；`recorder-view.test.ts` 的 `centerHintText` 一组——全空空闲提示"演奏即录"、
  有内容 / 播放中 / 录制中不提示、未连接提示「未连接 MIDI 键盘」（有内容也提示）、失败原因照实说；
  浏览器端 `probe-recorder.mjs`——中央提示文案逐字 + 字号 ≥16px + 居中偏差、未连接时提示
  「未连接 MIDI 键盘」（无设备 / 拔出键盘）、按键自动开录（按钮变暂停、提示收起）、静默自动暂停、
  再次演奏继续、有内容后提示收起、清空后重现，详见 `20260913-recorder-auto-record.md` §5。
